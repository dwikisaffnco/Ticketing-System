...
const { fetchTickets, archiveTicket } = ticketStore;
...
const handleArchive = async (ticket) => {
  ...
  const result = await Swal.fire({
    icon: "warning",
    title: "Archive tiket?",
    text: `Tiket #${ticket.code} akan dipindahkan ke menu Archive.`,
  ...
  await archiveTicket(ticket.code);
  await fetchTickets(filters.value);
  ...
  await Swal.fire({
    icon: "success",
    title: "Berhasil",
    text: "Tiket berhasil di-archive",
    timer: 1500,
