...
const handleSubmit = async () => {
  ...
  try {
    const response = await axiosInstance.post("/admin/users", form.value);
    success.value = response?.data?.message ?? "Akun berhasil dibuat";
...
<template>
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-lg font-semibold text-gray-900">Buat Akun</h1>
        <p class="text-sm text-gray-500">Admin dapat membuat akun user atau admin.</p>
      </div>
    </div>
    ...
    <form class="mt-6 space-y-4 max-w-xl" @submit.prevent="handleSubmit">
      <div>
